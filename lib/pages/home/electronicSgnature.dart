...
class electronicSignaturePage extends StatefulWidget {
  ...
  electronicSignaturePage({Key? key, required this.YXQDataMap, required this.userInfoMap}) : super(key: key);

  @override
  _electronicSignaturePageState createState() => _electronicSignaturePageState(YXQDataMap: this.YXQDataMap, userInfoMap:this.userInfoMap);
}
...
class _electronicSignaturePageState extends State<electronicSignaturePage> {
  ...
  Map YXQDataMap ; //医信签数据map
  Map userInfoMap ; //患者信息map
  _electronicSignaturePageState({required this.YXQDataMap, required this.userInfoMap});
  ...
  void getNet_YXQSignData() async{
    ...
    String dataStr =
        "姓名：" +userInfoMap["name"].toString() +"," +"性别：" +userInfoMap["sex"].toString() +"," +"年龄：" +userInfoMap["age"].toString() +"," +"费用：自费,科别：全科";

    HttpRequest? request = HttpRequest.getInstance();
    var res = await request?.post(Api.YXQSignDataUrl,{
      "userId"          : "123124", //医护人员userid
      "transactionId"   : "1", //业务编号 处方id
    ...
    if (res['code'] == 200) {
      ...
      Navigator.push(context, MaterialPageRoute(builder: (context)=> PrescriptDetail(userInfoMap: userInfoMap, imageStr: YXQDataMap["signatureImg"],)));
